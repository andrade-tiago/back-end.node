...
describe('Function isUserActive', () => {
  it('should return true if user is an ActiveUser instance', () => {
    ...
    expect(isUserActive(activeUserInstance)).toBe(true);
  });

  it('should return false if user is not an ActiveUser instance', () => {
    const deletedUserInstance = new DeletedUser({
      id: new Uuid('970e1881-7d4b-4aec-9691-8f5c2db2c14e'),
      createdAt: new NonFutureDate(),
    });

    expect(isUserActive(deletedUserInstance)).toBe(false);
  });
});
